...
#  I usually use both a Mac and a Windows PC and in each one the working directory is different, 
#  so I detect S.O. to define this location
#setwd("D:/D/R/Coursera/Project")
...
# download.file(datasource,destFile)
...
ColumnNames <- c("SubjectId","Activity",as.character(features$V2))
...
allData <- cbind(allData[1],Activity,allData[3:ncol(allData)])
#
# ------------------------------------------------------------------------------------------
...
columnNames <- gsub("\\)", "",columnNames)
# keeping the CamelCase because I think is better to understand 
...
allDataSum <- ddply(allDataLong,c("SubjectId","Activity","variable"),summarise,value=mean(value))
# Returning to a format with multiple measurement columns
allDataFinal <- dcast(allDataSum,SubjectId + Activity ~ variable)
